...
kubectl --context "$MESH_CONTEXT" -n "$namespace" delete authorizationpolicy allow-only-destination-waypoint generator-from-control-plane runtime-from-control-plane evidence-from-control-plane broker-from-publishers
kubectl --context "$MESH_CONTEXT" -n "$namespace" delete networkpolicy forge-waypoint allow-ambient-kubelet-health-probes

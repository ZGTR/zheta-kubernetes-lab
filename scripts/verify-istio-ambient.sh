...
for policy in control-plane-paths internal-services broker-publish-subscribe allow-ambient-kubelet-health-probes; do kubectl --context "$MESH_CONTEXT" -n zheta-forge get networkpolicy "$policy" >/dev/null; done
[ "$scope" = enrollment ] && { echo 'Ambient enrollment, strict mTLS, HBONE, and health-probe policy are present.'; exit 0; }

kubectl --context "$MESH_CONTEXT" -n zheta-forge get authorizationpolicy control-plane-l4-boundary generator-l4-boundary runtime-l4-boundary evidence-l4-boundary broker-l4-boundary >/dev/null
for policy in allow-bounded-bypass-observation-egress allow-bounded-bypass-observation-ingress; do kubectl --context "$MESH_CONTEXT" -n zheta-forge get networkpolicy "$policy" >/dev/null; done
...
kubectl --context "$MESH_CONTEXT" -n zheta-forge get authorizationpolicy generator-from-control-plane runtime-from-control-plane evidence-from-control-plane broker-from-publishers >/dev/null
...
kubectl --context "$MESH_CONTEXT" -n zheta-forge get networkpolicy forge-waypoint >/dev/null

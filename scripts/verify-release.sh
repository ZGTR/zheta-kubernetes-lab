...
[ "$(grep -Ec '^  - name: helixworks-forge/(control-plane|generator|runtime|evidence|broker)$' "$manifest")" -eq 5 ]
for service in control-plane generator runtime evidence broker; do
  [ "$(grep -Ec "^    newName: [0-9]{12}\\.dkr\\.ecr\\.[a-z0-9-]+\\.amazonaws\\.com/helixworks-forge/$service$" "$manifest")" -eq 1 ]
done
rendered="$(kubectl kustomize "$(dirname "$manifest")")"
for service in control-plane generator runtime evidence broker; do
  grep -Eq "image: [0-9]{12}\\.dkr\\.ecr\\.[a-z0-9-]+\\.amazonaws\\.com/helixworks-forge/$service@sha256:[0-9a-f]{64}$" <<<"$rendered"
done

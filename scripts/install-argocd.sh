...
echo "Waiting for Argo CD to clone Git and reconcile the Zheta Application."
for attempt in $(seq 1 60); do
  sync_state="$(kubectl -n argocd get application zheta -o jsonpath='{.status.sync.status}' 2>/dev/null || true)"
  health_state="$(kubectl -n argocd get application zheta -o jsonpath='{.status.health.status}' 2>/dev/null || true)"
...
kubectl -n argocd describe application zheta >&2
exit 1

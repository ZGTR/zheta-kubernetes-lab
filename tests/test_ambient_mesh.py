...
class AmbientMeshSourceTest(unittest.TestCase):
    ...
    def test_waypoint_bypass_probe_is_bounded_live_evidence(self):
        ...
        mesh_attempt = probe.split('probe_started=', 1)[1].split('correlated=0', 1)[0]
        self.assertIn("connection.sendall", mesh_attempt)
        self.assertIn("connection.recv", mesh_attempt)
        self.assertIn("if response:", mesh_attempt)
        self.assertIn("unexpected HTTP response bytes", mesh_attempt)
        self.assertIn("except BrokenPipeError", mesh_attempt)
        self.assertIn("except ConnectionAbortedError", mesh_attempt)
        self.assertIn("timeout/reset/broken-pipe/aborted/zero-byte EOF", mesh_attempt)
        self.assertNotIn('socket.create_connection((os.environ["TARGET_IP"],8080),timeout=3)\nexcept', mesh_attempt)
